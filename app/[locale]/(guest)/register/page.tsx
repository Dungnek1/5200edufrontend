...
export default function RegisterPage() {
  ...
  const [isLoginView, setIsLoginView] = useState<boolean>(tabParam === "login");

  useEffect(() => {
    setIsLoginView(tabParam === "login");
  }, [tabParam]);
    ...
    <div className={`flex items-center justify-center py-6 sm:py-12 px-4 ${isLoginView ? 'pb-[200px] sm:pb-[240px]' : ''}`}>
      <div className="w-full max-w-md">
        ...
        <div className="w-full bg-white border-2 border-blue-600 rounded-xl sm:rounded-2xl shadow-lg overflow-hidden">
          ...
          <div className="flex w-full bg-blue-600 relative h-[50px] sm:h-[60px]">
            <button

              onClick={() => handleTabChange(true)}
              className={`relative flex-1 text-center font-bold ${isLoginView
